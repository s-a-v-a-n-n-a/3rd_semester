...
// void (*reaction_functions[shape_types_amount * shape_types_amount])(Shape_manager *engine, const Shape *par_first, const Shape *par_second) =
// {
...
void Shape_manager::tick(Screen_information *screen, const double delta_time)
{
	...
	for (size_t i = 0; i < shapes_amount; ++i)
	{
		...
		if (!(next_position.get_x() == next_position_copy.get_x() && next_position.get_y() == next_position_copy.get_y()))
			;// printf("WTF\n");
	...
	// for (size_t i = 0; i < shapes_amount; ++i)
	// {
	// 	shapes_array[i]->move(new_positions[i]);
	// }
	...
	for (size_t i = 0; i < shapes_amount; ++i)
	{
		...
		else if (!shapes.get_array()[i]->get_active())
		{
			shapes.slow_delete(i);
		}
	}
