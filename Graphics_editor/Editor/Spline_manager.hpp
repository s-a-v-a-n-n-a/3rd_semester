...
public:
	Spline_manager(const size_t par_type, const Vector_ll &par_position, Texture *par_texture, const size_t par_width, const size_t par_height)
	: Visual_object(par_type, par_position, par_texture, par_width, par_height)
	{
		...
		Spline *spline = create_spline(par_position + Vector_ll(0, DEFAULT_BUTTON_HEIGHT), par_width, par_height - DEFAULT_BUTTON_HEIGHT);
	}
	~Spline_manager() = default;

	Spline *create_spline(const Vector_ll &position, const size_t width, const size_t height)
	{
		Spline *spline = new Spline((size_t)Vidget_type::SPLINE, position, GREY, width, height);
